...
const Sidebar = ({ sidebarOpen, setSidebarOpen }) => {
  ...
  const adminRoleChecker = () => {
    if (
      loggedInUserData?.data?.role.toString().trim() === '0' ||
      loggedInUserData?.data?.role.toString().trim() === '1'
    ) {
      return true;
    } else {
      return false;
